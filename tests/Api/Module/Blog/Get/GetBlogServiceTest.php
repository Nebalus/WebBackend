...
use Nebalus\Webapi\Api\Module\Blog\Get\GetBlogService;
use Nebalus\Webapi\Api\Module\Blog\Get\GetBlogValidator;
...
class GetBlogServiceTest extends TestCase
{
    private MySqlBlogRepository $blogRepository;
    private GetBlogResponder $responder;
    private GetBlogService $service;
    private GetBlogValidator $validator;

    protected function setUp(): void
    {
        $this->blogRepository = $this->createMock(MySqlBlogRepository::class);
        $this->responder = $this->createMock(GetBlogResponder::class);
        $this->service = new GetBlogService($this->blogRepository, $this->responder);
        $this->validator = $this->createMock(GetBlogValidator::class);
    }
    ...
    #[Test]
    public function testExecuteReturnsBlogWhenSelfUserHasPermission(): void
    {
        $requestingUser = $this->createMock(UserAccount::class);
        ...
        $this->validator->method('withContent')->willReturn(true);
    ...
    public function testExecuteReturnsNotFoundWhenBlogDoesNotExist(): void
    {
        ...
        $this->blogRepository->method('findBlogById')->willReturn(null);
