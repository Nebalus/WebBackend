...
use Nebalus\Webapi\Api\Admin\Role\Permission\Upsert\UpsertRolePermissionService;
...
class UpsertRolePermissionServiceTest extends TestCase
{
    private MySqlRoleRepository $repository;
    private UpsertRolePermissionResponder $responder;
    private UpsertRolePermissionService $service;
    ...
    protected function setUp(): void
    {
        ...
        $this->responder = $this->createMock(UpsertRolePermissionResponder::class);
        $this->service = new UpsertRolePermissionService($this->repository, $this->responder);
        $this->validator = $this->createMock(UpsertRolePermissionValidator::class);
    ...
    #[Test]
    public function testExecuteUpsertsPermissionsAndReturnsSuccess(): void
    {
        $this->userPerms->method('hasAccessTo')->willReturn(true);
        $roleId = RoleId::from(1);
        $this->validator->method('getRoleId')->willReturn($roleId);
        ...
        $permissionLinks = PermissionRoleLinkCollection::fromObjects($link1);
        $this->validator->method('getPermissionRoleLinks')->willReturn($permissionLinks);
        ...
        $this->repository->expects($this->exactly(2))
            ...
            ->willReturnOnConsecutiveCalls(
                PermissionRoleLinkCollection::fromObjects(), // Before
                PermissionRoleLinkCollection::fromObjects($link1) // After
            );

        $this->repository->expects($this->once())
            ->method('upsertPermissionLinksToRoleByRoleId')
            ->with($roleId, $permissionLinks);
