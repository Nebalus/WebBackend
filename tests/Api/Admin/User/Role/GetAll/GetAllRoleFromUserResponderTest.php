...
class GetAllRoleFromUserResponderTest extends TestCase
{
    #[Test]
    public function testRenderReturnsSuccessResultWithCorrectData(): void
    {
        ...
        $collection = RoleCollection::fromObjects($role1, $role2);

        $responder = new GetAllRoleFromUserResponder();
        $result = $responder->render($collection);

        $this->assertEquals(StatusCodeInterface::STATUS_OK, $result->getStatusCode());
        $this->assertEquals('Fetched all roles from this user', $result->getMessage());
